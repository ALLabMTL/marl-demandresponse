...
describe('SimulationManagerService', () => {
  const originalHousesData: HouseData[] = [
    ...
    { id: 3, hvacStatus: 'Lockout', secondsSinceOff: 1800, indoorTemp: 23, targetTemp: 22, tempDifference: 1 },
    { id: 4, hvacStatus: 'ON', secondsSinceOff: 20, indoorTemp: 20, targetTemp: 22, tempDifference: -2 },
  ];
  ...
  it('should sort housesData by tempDifference in increasing order', () => {
    const housesData = [
      { id: 1, hvacStatus: 'OFF', secondsSinceOff: 0, indoorTemp: 70, targetTemp: 72, tempDifference: 2 },
      { id: 2, hvacStatus: 'ON', secondsSinceOff: 0, indoorTemp: 68, targetTemp: 72, tempDifference: 4 },
      { id: 3, hvacStatus: 'Lockout', secondsSinceOff: 0, indoorTemp: 74, targetTemp: 72, tempDifference: 2 },
    ];
  ...
  it('should filter houses by multiple hvac status when more than one status is checked', () => {
    ...
    expect(service.isHvacChecked).toBeTrue();
    expect(service.hvacChosen).toEqual([{ id: 1, hvacStatus: 'ON', secondsSinceOff: 10, indoorTemp: 25, targetTemp: 22, tempDifference: 3 }, { id: 4, hvacStatus: 'ON', secondsSinceOff: 20, indoorTemp: 20, targetTemp: 22, tempDifference: -2 }, { id: 2, hvacStatus: 'OFF', secondsSinceOff: 600, indoorTemp: 22, targetTemp: 22, tempDifference: 0 }]);
   // expect(service.isFilteredHvac).toBeTrue();
